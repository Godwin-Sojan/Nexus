...
class SettingsFrame(ctk.CTkFrame):
    def __init__(self, master, logout_callback, system_frame, notes_frame):
        ...
        self.data_lbl.pack(pady=(30, 10))

        self.clear_notes_btn = ctk.CTkButton(self.container, text="Clear My Notes", width=200, height=40, font=FONT_BODY,
                                             fg_color=COLOR_SECONDARY, hover_color=COLOR_PRIMARY,
                                             command=self.clear_notes)
        self.clear_notes_btn.pack(pady=10)

        # --- Account ---
        self.logout_btn = ctk.CTkButton(self.container, text="Logout", width=200, height=40, font=FONT_BODY,
                                        fg_color=COLOR_DANGER, hover_color="#8a1f15",
                                        command=self.logout_callback)
        self.logout_btn.pack(pady=(40, 10))
        
        # --- About ---
        self.about_lbl = ctk.CTkLabel(self.container, text="v1.0 | Developed by Godwin", font=FONT_SMALL, text_color="gray60")
        self.about_lbl.pack(side="bottom", pady=20)
